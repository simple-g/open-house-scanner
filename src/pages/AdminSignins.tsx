  ...
  const exportCsv = () => {
    ...
    const bodyLines = rows.map(r => {
      const vals = [
        ...
        r.phone ?? '',
        r.role ?? '',
        typeof r.timeline === 'string' ? r.timeline : JSON.stringify(r.timeline ?? '')
      ]
      // Escape " by doubling them, and wrap each field in quotes
  ...
  return (
    <div style={{ maxWidth: 900, margin: '24px auto', padding: 16 }}>
      ...
      <table style={{ width: '100%', marginTop: 12, borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            {['Time','Name','Email','Phone','Role','Timeline'].map(h => (
              <th key={h} style={{ textAlign:'left', borderBottom:'1px solid #ddd', padding:'6px 4px' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
